...
public class SelectCharStyled : MonoBehaviour {
	...
	public UMADynamicAvatar[] charsPrefabs;
	...
	private static bool toSwitch = false;
	...
	void Start() {
		...
		int index = 0;
		Debug.Log ("length "+ charsPrefabs.Length);
		foreach (UMADynamicAvatar t in charsPrefabs) {
	...
	private void DoSignUp()
	{
		...
		if (this.password!=string.Empty && this.password == this.repeatPassword) {
			...
			UMA.UMAData.UMARecipe r = chars[currentChar].GetComponent<UMAAvatarBase>().umaData.umaRecipe;
			UMAContext c = chars[currentChar].GetComponent<UMAAvatarBase>().context;
			asset.Save(r, c);
			string config = asset.recipeString;

			Debug.Log("config saved: " + config);

			//save to server
			ParseObject user = new ParseObject("IIMe");
			user["username"] = UserName;
			user["password"] = password;
			user["email"] = email;
			//user[DB_avatarCustomization.prefAvatar] = avatarIdx.ToString();
			user[DB_avatarCustomization.prefConfig] = config;
